...
class Admin(commands.Cog):
    ...
    async def kick(self, ctx, user: str = None, *, reason: str = None):
        if not ctx.author.guild_permissions.kick_members:
            return await ctx.send('🚨 당신은 이 명령어를 사용 할 권한이 부족합니다. `유저 추방하기` 권한을 추가해주세요! 🚨')
        if not user:
            return await ctx.send('🚨 추방할 유저 맨션 또는 아이디를 입력해주세요. 🚨')
        if reason is None:
            reason = '사유를 적지 않음'
        try:
        ...
        embed = discord.Embed(title='🚨유저 추방 완료🚨', color=0xDDEED,
